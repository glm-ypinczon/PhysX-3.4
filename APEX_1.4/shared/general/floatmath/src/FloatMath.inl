...
public:
  ...
  void Tridiagonal(void)
  {
    ...
    if (fM02 != (Type)0.0)
    {
      Type fLength = static_cast<REAL>(::sqrt(fM01*fM01+fM02*fM02));
  ...
  bool QLAlgorithm(void)
  {
    ...
    for (int32_t i0 = 0; i0 <3; i0++)
    {
      ...
      for (i1 = 0; i1 < iMaxIter; i1++)
      {
        ...
        for (i2 = i0; i2 <= (3-2); i2++)
        {
          Type fTmp = static_cast<REAL>(fabs(m_afDiag[i2]) + fabs(m_afDiag[i2+1]));
        ...
        Type fR = static_cast<REAL>(::sqrt(fG*fG+(Type)1.0));
        if (fG < (Type)0.0)
        {
        ...
        for (int32_t i3 = i2-1; i3 >= i0; i3--)
        {
          ...
          {
            fCos = fG/fF;
            fR = static_cast<REAL>(::sqrt(fCos*fCos+(Type)1.0));
            m_afSubd[i3+1] = fF*fR;
            fSin = ((Type)1.0)/fR;
            fCos *= fSin;
          }
          ...
          {
            fSin = fF/fG;
            fR = static_cast<REAL>(::sqrt(fSin*fSin+(Type)1.0));
            m_afSubd[i3+1] = fG*fR;
            fCos = ((Type)1.0)/fR;
            fSin *= fCos;
          }
...
void fm_quickSort::qsort(void **b,int32_t num)
{
	char *lo,*hi;
	char *mid;
	char *bottom, *top;
...
public:
  ...
  void init(const fm_LineSegment &s,fm_VertexIndex *vpool,uint32_t x)
  {
  ...
  void intersect(MyLineSegment *segment,uint32_t x,uint32_t y,uint32_t /* z */,fm_VertexIndex *vpool)
  {
    uint32_t count = 0;
...
public:
  ...
  fm_LineSegment * performLineSweep(const fm_LineSegment *segments,uint32_t icount,const double *planeEquation,fm_VertexIndex *pool,uint32_t &scount)
  {
    ...
    qsort((void **)mptr,(int32_t)icount);

    for (uint32_t i=0; i<icount; i++)
    {
      MyLineSegment *segment = mptr[i];
      double esegment = segment->mTo[mX];
      for (uint32_t j=i+1; j<icount; j++)
      {
	...
	int32_t compare(void **p1,void **p2)
  {
    int32_t ret = 0;
  ...
  uint32_t              mX;  // index for the x-axis
  uint32_t              mY;  // index for the y-axis
  uint32_t              mZ;
...
{
  REAL A=0.0f;
  for(uint32_t p=pcount-1,q=0; q<pcount; p=q++)
  {
...
uint32_t  fm_copyUniqueVertices(uint32_t vcount,const REAL *input_vertices,REAL *output_vertices,uint32_t tcount,const uint32_t *input_indices,uint32_t *output_indices)
{
  ...
  uint32_t *reindex = (uint32_t *)PX_ALLOC(sizeof(uint32_t)*vcount, PX_DEBUG_EXP("fm_copyUniqueVertices"));
...
public:
  ...
  const uint32_t * tesselate(fm_VertexIndex *vindex,uint32_t tcount,const uint32_t *indices,float longEdge,uint32_t maxDepth,uint32_t &outcount)
  {
    ...
    if ( mVertices->isDouble() )
    {
      ...
      double *vertices = (double *)PX_ALLOC(sizeof(double)*vcount*3, PX_DEBUG_EXP("fm_Tesselate"));
    ...
    {
      uint32_t vcount = mVertices->getVcount();
      float *vertices = (float *)PX_ALLOC(sizeof(float)*vcount*3, PX_DEBUG_EXP("fm_Tesselate"));
...
void CTriangulator::_process(TU32Vector &indices)
{
    ...
    if (n < 3)
        return;
    int32_t *V = (int32_t *)PX_ALLOC(sizeof(int32_t)*n, PX_DEBUG_EXP("CTriangulator"));
    ...
    for (int32_t m = 0, v = nv - 1; nv > 2;)
    {
        ...
        if (_snip(u, v, w, nv, V))
        {
			...
			if ( flipped )
			{
				indices.pushBack((uint32_t)a);
				indices.pushBack((uint32_t)b);
				indices.pushBack((uint32_t)c);
			}
			else
			{
				indices.pushBack((uint32_t)c);
...
bool CTriangulator::_snip(int32_t u, int32_t v, int32_t w, int32_t n, int32_t *V)
{
    ...
    const TVec &A = mPoints[ (uint32_t)V[(uint32_t)u] ];
    const TVec &B = mPoints[ (uint32_t)V[(uint32_t)v] ];
    const TVec &C = mPoints[ (uint32_t)V[(uint32_t)w] ];
    ...
    for (p = 0; p < n; p++)
    {
        if ((p == u) || (p == v) || (p == w))
            continue;
        const TVec &P = mPoints[ (uint32_t)V[(uint32_t)p] ];
...
public:
                                             ...
                                             double epsilon)
  {
    ...
    double *points = (double *)PX_ALLOC(sizeof(double)*pcount*3, PX_DEBUG_EXP("fm_Triangulate"));
    if ( consolidate )
    {
    ...
    if ( pcount >= 3 )
    {
      ...
      {
        tcount = _tcount;
        mPointsDouble = (double *)PX_ALLOC(sizeof(double)*tcount*3*3, PX_DEBUG_EXP("fm_Triangulate"));
                                             ...
                                             float epsilon)
  {
    ...
    double *temp = (double *)PX_ALLOC(sizeof(double)*pcount*3, PX_DEBUG_EXP("fm_Triangulate"));
    double *dest = temp;
    for (uint32_t i=0; i<pcount; i++)
    {
    ...
    if ( results )
    {
      ...
      mPointsFloat = (float *)PX_ALLOC(sizeof(float)*tcount*3*3, PX_DEBUG_EXP("fm_Triangulate"));
